...
public interface MarketplaceProductController {
    ...
    ResponseEntity<String> addProduct(
            @NotBlank String marketplaceCode,
            @NotBlank String categoryName,
            @Valid MarketplaceProductRequest request
    );
    ...
    @ApiResponse(responseCode = "400", description = "Validation error",
            content = @Content(schema = @Schema(implementation = ErrorData.class)))
    ResponseEntity<List<ProductResponse>> searchProducts(@Valid ProductSearchRequest request);
}

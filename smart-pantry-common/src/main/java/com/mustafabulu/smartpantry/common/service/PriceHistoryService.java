...
public class PriceHistoryService {
    ...
    private static final String OPPORTUNITY_LEVEL_NORMAL = "Normal";
    ...
    public List<PriceHistoryResponse> buildProductPriceResponses(
            Product product,
        ...
        return normalizedHistories.stream()
                .map(entry -> {
                    ...
                    return new PriceHistoryResponse(
                            ...
                            availabilityScore,
                            opportunityLevelsByMarketplace.getOrDefault(
                                    history.getMarketplace(),
                                    OPPORTUNITY_LEVEL_NORMAL
                            ),
    ...
    private NormalizedPriceHistory toNormalizedHistory(
        ...
        return new NormalizedPriceHistory(
                history,
                MarketplacePriceNormalizer.normalizeForDisplay(selectedPrice)
        );
    }
    ...
    private Double calculateCrossMarketScore(
        ...
        double boundedRatio = Math.clamp(ratio, CROSS_MARKET_RATIO_MIN, CROSS_MARKET_RATIO_MAX);
    ...
    private String resolveOpportunityLevel(List<BigDecimal> prices) {
        if (prices == null || prices.size() < 6) {
            return OPPORTUNITY_LEVEL_NORMAL;
        }
